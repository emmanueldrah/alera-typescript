...
const ImagingDashboard = () => {
  ...
  const newRequests = imagingScans.filter((scan) => scan.status === 'requested');
  const inProgress = imagingScans.filter((scan) => scan.status === 'in-progress');
  const completed = imagingScans.filter((scan) => scan.status === 'completed');
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          {imagingScans.length === 0 ? (
            ...
            <div className="space-y-3">
              {imagingScans.slice(0, 3).map((scan) => (
