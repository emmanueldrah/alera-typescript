...
import { FlaskConical, Clock, CheckCircle, ArrowRight, AlertCircle, Inbox } from 'lucide-react';
...
const LaboratoryDashboard = () => {
  ...
  const newRequests = labTests.filter((test) => test.status === 'requested');
  const inProgress = labTests.filter((test) => test.status === 'in-progress');
  const completed = labTests.filter((test) => test.status === 'completed');
    ...
    <div className="space-y-6">
      ...
      <motion.div {...card(3)} className="bg-card rounded-2xl border border-border p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-display font-semibold text-card-foreground">All Test Requests</h2>
          ...
          {labTests.length === 0 ? (
            ...
            <div className="space-y-3">
              {labTests.slice(0, 3).map((test) => (
                <div key={test.id} className="rounded-lg border border-border/50 bg-secondary/50 p-3">
                  <div className="text-sm font-medium text-card-foreground">{test.testName}</div>
                  <div className="mt-1 text-xs text-muted-foreground">{test.patientName} • {test.status}</div>
                </div>
              ))}
            </div>
          )}
